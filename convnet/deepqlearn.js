...
(function (global) {
  ...
  var Brain = function (num_states, num_actions, opt) {
    ...
    // update target_net every C steps.
    this.update_target_iteratioin = typeof opt.update_target_iteratioin !== 'undefined' ? opt.update_target_iteratioin : 10000;
    ...
    this.net_inputs = num_states * this.temporal_window + num_states;
    ...
    if (typeof opt.layer_defs !== 'undefined') {
      ...
      if (layer_defs[0].out_depth * layer_defs[0].out_sx * layer_defs[0].out_sy !== this.net_inputs) {
        console.log('TROUBLE! Number of inputs must be num_states * temporal_window + num_states!');
      }
    ...
    // Allocate Q_hat.
    this.target_net = new convnetjs.Net();
    this.target_net.fromJSON(this.value_net.toJSON());
  ...
  Brain.prototype = {
    ...
    getNetInput: function (xt) {
      // return s = (x, x, x, xt) state vector. 
    ...
    forward: function (input_array) {
      ...
      if (this.forward_passes > this.temporal_window) {
        ...
        if (rf < this.epsilon) {
          ...
          var maxact = this.policy(net_input, this.value_net);
    ...
    backward: function (reward) {
      ...
      if (this.age >= this.learning_steps_total) {
        this.learning = false;
        return;
      }
      ...
      if (this.experience.length > this.start_learn_threshold) {
        ...
        for (var k = 0; k < this.tdtrainer.batch_size; k++) {
          ...
          if (e.reward0 > 0) {
            ...
            // Use the target network to compute the target value.
            var maxact = this.policy(e.state1, this.target_net);
            r += this.gamma * maxact.value;
          }
        ...
        // Every update_target_iteration, update the target_net.
        if ((this.age + 1) % this.update_target_iteratioin == 0) {
          this.target_net.fromJSON(this.value_net.toJSON());
        }
      }
    },
