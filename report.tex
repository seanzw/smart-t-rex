...
\subsubsection{Bucketizing the perceptions to features}

First we need to encode the environment, i.e. perceptions of the TRex, into several discrete states. Since the variables in the perception are floating point numbers, we use a certain number of buckets to discretize them. The bucketizing process for each variable can be described as followed.
...
CANVAS\_WIDTH and CANVAS\_HEIGHT represent the width and the height of the canvas, respectively. BUCKET denotes the bucket size for that variable. Empirically, BUCKET\_X = 20, BUCKET\_H = 10, BUCKET\_V = 10. MAX\_V and MIN\_V denote the maximal velocity and minimal velocity of the horizon, and their values are 13 and 6 respectively. \textit{Offset} is a variable denoting the speed offset of the obstacle. For an obstacle belong to the type of cactus, \textit{offset} = 0; for a pterodactyls obstacle, \textit{offset} can be either 0.8 or -0.8 with equal probability.
...
\begin{enumerate}
    ...
    (BUCKET\_X * BUCKET\_H + 1)
    \item X Position + HEIGHT + VELOCITY 

    (BUCKET\_X * BUCKET\_H * BUCKET\_V + 1)
\end{enumerate}
...
For a state in which the TRex collided with an obstacle, the reward was set to -1000. When the TRex is running on the horizon, the reward is 0. When the TRex jumps over an obstacle and lands on the horizon, the reward is 1.
...
First we use the same equation as Eq.~\ref{eq:x_bucket}, Eq.~\ref{eq:h_bucket} and Eq.~\ref{eq:v_bucket} to normalize the data. Notice that for neural network the input can be a continuous value. Therefore, we do not have to bucketizing the features to discrete values, and the value for each feature is within range $[0, 1]$. For example, the X position of the first obstacle is normalized with
\begin{equation}
    \hat{x}=\frac{x}{CANVAS\_WIDTH}
\end{equation}

\subsubsection{Training Detail}
As mentioned above, the most challenging part for Deep-Q-Learning is that the network may not converge. Here we implement the algorithm from DeepMind. There are a few details we want to mention for training. First, the agent of Deep-Q-Learning does not make decision for each frame. Instead, it only decides whether to jump or not every 6 frames. This is to make sure that neighboring samples used for training differ from each other and make the training process more stable. 
Another thing to notice is that as mentioned in the algorithm, we use an 

\section{Implementation}
Hack into the codes of an extracted version of TRex.
Front end. 
...
\subsection{Q-Learning}
    iterations needed to get a good result
    time needed
    max score
    average score
    The plot of the score increase during training. Score vs. game number
    Behavior of the TRex with different features.


\section{Conclusion}
Q-learning is sufficient to solve this game.
